...
class DataManager(private val player: GamePlayer) {
    ...
    private fun writeDatabase() {
        ...
        // 确保每条数据 只会产生一次提交
        val map: MutableMap<Any, DataAction> = IdentityHashMap()
        ...
        bizThread.execute {
            try {
                ...
                for (dataAction in map.values) {
                    ...
                    val uniqueFields: MutableList<FieldMessage> = ArrayList()
                    ...
                    if (dataAction.type == DataActionType.UPDATE) {
                        val updateFields: MutableList<FieldMessage> =
                            ArrayList()
                    ...
                    } else if (dataAction.type == DataActionType.INSERT) {
                        for ((key, value) in dataClassCache.updateFields) {
                            val fieldMessage = FieldMessage(key, value[dataAction.data])
                            uniqueFields.add(fieldMessage)
                        }
                        request.inserts.add(Insert(tableName, uniqueFields))
                    } else if (dataAction.type == DataActionType.DELETE) {
