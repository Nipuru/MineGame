...
import top.nipuru.minegame.game.gameplay.chat.formatter.MessageFormatter
import top.nipuru.minegame.game.gameplay.chat.formatter.MessagePattern
import top.nipuru.minegame.game.gameplay.chat.formatter.PlainMessageFormatter
import top.nipuru.minegame.game.gameplay.chat.formatter.ShowItemMessageFormatter
import top.nipuru.minegame.game.gameplay.player.GamePlayer
import java.util.*
import java.util.regex.Matcher

object MessageFormat {
    private val formatters: MutableList<MessageFormatter> = ArrayList()
    private val plainFormatter: MessageFormatter = PlainMessageFormatter()
    ...
    fun parse(sender: GamePlayer, message: String): Array<FragmentMessage> {
        ...
        for (i in formatters.indices) {
            ...
            while (matcher.find()) {
                ...
                val fragmentMessage = FragmentMessage(i, arrayOf(fragment.args))
        ...
        while (i < message.length) {
            ...
            if (plainFragment != null) fragments.add(FragmentMessage(plainFormatterIdx, arrayOf(plainFragment.args)))
        ...
        if (start < message.length) {
            ...
            if (plainFragment != null) {
                fragments.add(FragmentMessage(plainFormatterIdx, arrayOf(plainFragment.args)))
            }
        }
    ...
    init {
        ...
        formatters.add(ShowItemMessageFormatter())
    }
}
